    ...
    <article className="bg-bg-surface border border-border-subtle rounded-[var(--radius)] overflow-hidden transition-all duration-200">
      ...
      >
        <div className="text-sm font-medium text-text-primary flex items-center gap-2.5">
          <span className="font-mono text-caption font-bold w-6 h-6 rounded-md flex items-center justify-center bg-accent-magenta-dim text-accent-magenta shrink-0">
            Q{index}
          </span>
          {question}
        </div>
      ...
      >
        <div className="overflow-hidden">
          <div className="px-6 pt-4 pb-6 pl-[58px] text-[13px] text-text-secondary leading-[1.8] [&_ol]:list-decimal [&_ol]:pl-5 [&_ol]:my-2 [&_ol]:flex [&_ol]:flex-col [&_ol]:gap-1.5 [&_ul]:list-disc [&_ul]:pl-5 [&_ul]:my-2 [&_ul]:flex [&_ul]:flex-col [&_ul]:gap-1.5">
            {children}
          </div>
        </div>
      </div>
