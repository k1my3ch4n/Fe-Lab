...
import { TypeGuardDemo } from "@features/type-guard";
import { codeExamples, interviewQuestions } from "@features/type-guard/data";

export default function TypeGuardPage() {
  return (
    <>
      <Topbar topicId="type-guard" />

      <ConceptSection
        title="타입 가드와 타입 좁히기"
        description="타입 가드는 런타임 검사를 통해 TypeScript가 특정 스코프 내에서 타입을 더 구체적으로 추론하도록 합니다."
      >
      ...
      <DemoSection
        title="타입 좁히기 시각화"
        description="각 타입 가드를 선택하고 실행하여 타입이 어떻게 좁혀지는지 확인하세요."
      >
        <TypeGuardDemo />
      </DemoSection>
